...
function initTabContainer(container: HTMLElement): () => void {
  ...
  if (!synthesize) {
    const triggers = Array.from(
      container.querySelectorAll<HTMLElement>("[data-nb-tabs-trigger]"),
    ).filter((trigger) => trigger.closest("[data-nb-tabs]") === container);
    const panels = Array.from(
      container.querySelectorAll<HTMLElement>("[data-nb-tabs-content]"),
    ).filter((panel) => panel.closest("[data-nb-tabs]") === container);
    const panelByValue = new Map(panels.map((panel) => [panel.dataset.nbValue, panel]));
    const orderedPanels = triggers.map((trigger) => panelByValue.get(trigger.dataset.nbValue));
    const parent = panels[0]?.parentElement;

    if (
      parent &&
      orderedPanels.every((panel) => panel?.parentElement === parent) &&
      new Set(orderedPanels).size === panels.length
    ) {
      for (const panel of orderedPanels) {
        if (panel) parent.append(panel);
      }
    }
  }
